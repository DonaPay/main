...
const CreateProfile = () => {
      ...
      if (userImage) {
        ...
        if (trxObject) {
          ...
          return toast.promise(waitForTransactionConfirmation(txn.hash), {
            loading: "User Creation in process",
            success: "User Created Successfully",
            error: "User Creation Failed",
          });
          setSection("");
        }
      } else toast.warning("No image provided");
    } catch (error) {
  ...
  useEffect(() => {
    if (section === "create-profile") {
      console.log("User is on create profile section");
    }
  }, [section]);
    ...
    <div className="w-full h-full flex flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="w-full max-w-3xl mx-auto font-light text-sm text-right">
          Your profile photo gets securely stored on the Arweave Network.
        </p>
      </div>
